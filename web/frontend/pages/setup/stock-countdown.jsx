...
export default function StockCountdown() {
    ...
    // Default values for new entries only
    const getDefaultData = () => ({
        ...
        progressBarPosition: 'below',
        progressBarWidth: 100,
        progressBarHeight: 8,
        progressBarBackgroundColor: { hue: 0, saturation: 0, brightness: 0.9 },
        progressBarBackgroundHexColor: '#e1e3e5',
        // Design settings - no defaults, will be set by user
        backgroundColor: { hue: 0, saturation: 0, brightness: 0.9 },
        hexColor: '#ffffff',
        borderColor: { hue: 0, saturation: 0, brightness: 0.8 },
        cardBorderRadius: 0, // Start with 0, user will set their preferred value
        cardBorderSize: 0, // Start with 0, user will set their preferred value
    ...
    // Saved data state - use defaults only for new entries
    const [savedData, setSavedData] = useState(getDefaultData());
    ...
    useEffect(() => {
        if (id) {
            const loadExistingData = async () => {
                try {
                    const response = await fetchWithAuth(`/api/stock-countdown/edit/${id}`);
                    if (response.ok) {
                        const data = await response.json();
                        console.log("📥 Loading existing data:", data);
                        
                        // Map the database fields to form fields - use exact saved values
                        const loadedData = {
                            title: data.title ?? '',
                            isPublished: data.is_published ?? false,
                            stockLimit: data.stock_limit ?? 10,
                            messageTemplate: data.message_template ?? "Only {{stock}} left.",
                            selectedVariants: data.selected_variants ?? [],
                            showProgressBar: data.show_progress_bar ?? false,
                            barColor: data.bar_color ?? { hue: 120, saturation: 1, brightness: 1 },
                            barHexColor: data.bar_hex_color ?? '#00ff00',
                            progressBarStyle: data.progress_bar_style ?? 'rounded',
                            progressBarPosition: data.progress_bar_position ?? 'below',
                            progressBarWidth: data.progress_bar_width ?? 100,
                            progressBarHeight: data.progress_bar_height ?? 8,
                            progressBarBackgroundColor: data.progress_bar_background_color ?? { hue: 0, saturation: 0, brightness: 0.9 },
                            progressBarBackgroundHexColor: data.progress_bar_background_hex_color ?? '#e1e3e5',
                            // Design settings from design_config - use exact saved values
                            backgroundColor: data.design_config?.backgroundColor ?? { hue: 0, saturation: 0, brightness: 0.9 },
                            hexColor: data.design_config?.hexColor ?? '#ffffff',
                            borderColor: data.design_config?.borderColor ?? { hue: 0, saturation: 0, brightness: 0.8 },
                            cardBorderRadius: data.design_config?.cardBorderRadius ?? 0, // Use exact saved value
                            cardBorderSize: data.design_config?.cardBorderSize ?? 0, // Use exact saved value
                            cardBorderColor: data.design_config?.cardBorderColor ?? '#333333',
                            borderHexColor: data.design_config?.borderHexColor ?? '#cccccc',
                            insideTop: data.design_config?.insideTop ?? 10,
                            insideBottom: data.design_config?.insideBottom ?? 10,
                            outsideTop: data.design_config?.outsideTop ?? 0,
                            outsideBottom: data.design_config?.outsideBottom ?? 0,
                            messageSize: data.design_config?.messageSize ?? 16,
                            messageColor: data.design_config?.messageColor ?? { hue: 0, saturation: 0, brightness: 0.13 },
                            messageHexColor: data.design_config?.messageHexColor ?? '#202223',
                            // Placement settings from placement_config
                            displayPosition: data.placement_config?.display_position ?? 'after-price-before-quantity',
                            displayPages: data.placement_config?.display_pages ?? 'product-only',
                        };
    ...
    const handleSave = async () => {  
        ...
        try {
            const response = await fetchWithAuth("/api/stock-countdown/save", {
                method: "POST",
                headers: {
            ...
            if (response.ok) {
                ...
                setBannerErrors([]);
                setSuccessMessage(`${formData.title?.trim() || 'Stock Countdown'} was created successfully.`);
                setSavedData(formData);
                setIsDirty(false);
                toast.show('Stock Countdown Created', { duration: 3000 });
                window.scrollTo({ top: 0, behavior: 'smooth' });
                setTimeout(() => navigate("/"), 2000);
            } else {
                const errorData = await response.json();
                const msg = (errorData && (errorData.message || errorData.error)) || 'Failed to save. Please try again.';
                setBannerErrors([msg]);
                setSuccessMessage('');
    ...
    return (
        <div style={{ height: '100vh' }}>
            <Frame>
                ...
                <Page
                    title="Stock Countdown Settings"
